...
//	unsigned  char number[8];
	...
	if(res_sd == ERR_OK )
	{
		...
		cut_between_strs(FatFile_URL.FileHead,(unsigned  char*)"\"toolname\":\"",(unsigned  char*)"\"", device.Toolname,sizeof(device.Toolname),&u16temp); //¶ÁSD¿¨ÄÚÈÝ
		cut_between_strs(FatFile_URL.FileHead,(unsigned  char*)"\"province\":\"",(unsigned  char*)"\"", FatFile_URL.FileSourcePath,sizeof(FatFile_URL.FileSourcePath),&u16temp); //¶ÁSD¿¨ÄÚÈÝ
		device.province = atoi((char*)FatFile_URL.FileSourcePath);  //Ê¡·Ý		
	}
	res_sd=f_close(&fsrc);
...
//	unsigned int LocalPointSize;
	...
	//					tpcb->state = 9;
			...
			else if(ChargeM_init.NetState==NET_SOCKET_OK || ChargeM_init.NetState==NET_SOCKET_HEART_OK)  //SOCKET³É¹¦
			{
				if(tpcb->state ==ESTABLISHED)   
				{
...
//			if (strcmp((unsigned  char*)LocalPoint, (const char *)"?")== 0)
		...
		{
			i = cut_between_strs(cmd_bufer,(unsigned  char*)"\"sd\":\"",(unsigned  char*)"\"", FatFile_URL.FileSourcePath,sizeof(FatFile_URL.FileSourcePath),&j); //¶ÁSDÄ¿Â¼
			if(strlen((char*)FatFile_URL.FileSourcePath))
			{
				if (f_opendir(&dirs, (const TCHAR*)FatFile_URL.FileSourcePath) == FR_OK) 
				{
					...
					{														
						if(!finfo1.fname[0])	  //ÎÄ¼þÃû²»Îª¿Õ£¬Èç¹ûÎª¿Õ£¬Ôò±íÃ÷¸ÃÄ¿Â¼ÏÂÃæµÄÎÄ¼þÒÑ¾­¶ÁÍêÁË
						{
							break;								
						}         							
//						if(finfo1.fattrib&AM_DIR||finfo1.fattrib&AM_ARC) //ÎÄ¼þÀàÐÍ
						{
							sprintf((char*)LocalPoint, "(%X)%s size=%dB (%s)\r\n",finfo1.fattrib,finfo1.fname,(unsigned int)finfo1.fsize, finfo1.lfname);
							udp_demo_senddata(upcb,LocalPoint);	 
							Delay_10ms(2);	                                   //ÑÓÊ±10ms
						}
					}
					finfo1.lfname =0;
					finfo1.lfsize =0;
		...
		else if(cmd_bufer[0] =='d'&&cmd_bufer[1] =='e'&&cmd_bufer[2] =='l'&&cmd_bufer[3] =='e')		//É¾³ý
		{
			i = cut_between_strs(cmd_bufer,(unsigned  char*)"\"sd\":\"",(unsigned  char*)"\"", FatFile_URL.FileSourcePath,sizeof(FatFile_URL.FileSourcePath),&j); //delete SD¿¨ÄÚÈÝ
			if(strlen((char*)FatFile_URL.FileSourcePath))
			{
...
//						res_sd = f_read (&fsrc, FatFile_URL.FileBuf, FatFile_URL.FileBufSize, &fnum);
		...
		{
			i = cut_between_strs(cmd_bufer,(unsigned  char*)"\"chargetime\":\"",(unsigned  char*)"\"", LocalPoint,LocalPointSize,&j);		 //POWER  ÉÏµç 	
//				if (strcmp((unsigned  char*)LocalPoint, (const char *)"?")== 0)
			if(strlen((char*)LocalPoint))
			{
			...
			i = cut_between_strs(cmd_bufer,(unsigned  char*)"\"ssi\":\"",(unsigned  char*)"\"", FatFile_URL.FileHead,sizeof(FatFile_URL.FileHead),&j);		 //·þÎñÆ÷ÐÅÏ¢	
			...
			i = cut_between_strs(cmd_bufer,(unsigned  char*)"\"device\":{",(unsigned  char*)"}",FatFile_URL.FileHead,sizeof(FatFile_URL.FileHead),&j);		 //Éè±¸ºÅµÈÉèÖÃ
			if(strlen((char*)FatFile_URL.FileHead))
			{
			...
			i = cut_between_strs(cmd_bufer,(unsigned  char*)"\"udp\":\"",(unsigned  char*)"\"",FatFile_URL.FileHead,sizeof(FatFile_URL.FileHead),&j);		 //¹Ø±ÕUDP
...
//³õÊ¼»¯Ê±¼ä·þÎñÆ÷
	...
	sprintf((char*)HttpU_buffer[2].type , "S30");  //URLÀàÐÍ
...
//³õÊ¼»¯Ä¬ÈÏADS
	...
	sprintf((char*)HttpU_buffer[3].type , "S40");  //URLÀàÐÍ
...
//	unsigned int LocalPointSize;
	...
	if(LCDC.SW_state  == SW_NULL)
	{
	  ...
	  LCDC.TimeTimerSet =1000;
		...
		for(i=0;i<MAX_AD_NUMBER;i++)  //¹ã¸æ¼ÆÊýÇå0
		{
			LCDC.AD[i].count =0;
		}		
...
//					LCDC.ADTime[i] = time_sys;
			...
			if((LCDC.Mode[i])==LCD_MODE1)  //³äµçÄ£Ê½
			{
				if(time_sys-LCDC.TimeTimer[i]>LCDC.TimeTimerSet && (LCDC.LcdNumber[i]>0))
				{			
					LCDC.TimeTimer[i] +=LCDC.TimeTimerSet;
...
//					if(device.PortPowerUseTime[i]>0)
		...
		//						FatFile_AD.f_unm = LCDC.SPID[i]; 
							...
							LCDC.ADTime[i] = time_sys;      //¹ã¸æÊ±¼äÍ¬²½
							LCDC.TimeTimer[i] =time_sys;
							continue;
						}
						if(FatFile_AD.SW_state == SW_OFF)  //¹Ø±ÕÊ±ÉêÇëÊ¹ÓÃ
...
//							display_flash_BMPE (14,195,3,((Uport_PowerUseTime[0]%10)+'0'),3);//?? ?L							
						...
						if(time_sys - LCDC.ADTime[i]>LCDC.ADTimeSet)  //¹ã¸æ²¥·Å
						{
							...
							if(FatFile_AD.SW_state == SW_OFF)  //¹Ø±ÕÊ±ÉêÇëÊ¹ÓÃ
							{
								...
								res_sd = f_close(&fsrc); 	 		
//								sprintf((char*)FatFile_AD.FileSourcePath, "area2/T%d.bmp",(device.PortPowerUseTime[i]%3600/600));
								bmp_print_unit(116,0,FatFile_AD,LCDC.LcdNumber[i]);//ÏÔÊ¾Í¼Æ¬µ¥Ôª¡£		
			...
			if((LCDC.Mode[i])==LCD_MODE2)  //¿ÕÏÐÄ£Ê½
			{
...
//						LCD_InitAll();
		...
		//					tft_DisplayStr(270, 125, device_num,0x0000,0xffff,1);	
			...
			if((LCDC.Mode[i])==LCD_MODE_TEST)  //²âÊÔÄ£Ê½
			{
				if(time_sys-LCDC.SPTime[i]>LCDC.SPTimeSet && (LCDC.LcdNumber[i]>0))
				{			
					LCDC.SPTime[i] = time_sys;					
